...
if [ $1 -gt 1 ]; then
    ...
    systemctl status neutron-openvswitch-agent > /dev/null 2>&1 || :
    ovs_agent_running=$?
    [ $ovs_agent_running -eq 0 ] && systemctl stop neutron-openvswitch-agent > /dev/null 2>&1 || :
    ...
    [ $ovs_agent_running -eq 0 ] && systemctl start neutron-openvswitch-agent > /dev/null 2>&1 || :
